...
import { errorSessionsEvent, updateSessionEvent } from "./constants";
...
export function ClassSessionEdition() {
  const { id } = useParams();
  ...
  useEffect(() => {
    try {
      dispatch(addPage({ name: "Edición de clase", url: location.pathname }));
      const socket = AcademicSessionsSocket.getSessionInstanceWithToken();
      socket.on(updateSessionEvent, updateSessionListener);
      socket.on(errorSessionsEvent, errorListener);
      return () => {
        socket.off(updateSessionEvent);
  ...
  const updateSessionListener = (event: any) => {
  ...
  return (
    <Container maxWidth="sm" sx={{ my: "6rem" }}>
      ...
      <motion.div
        ...
        exit={{ opacity: 0 }}
      >
        <ClassSessionForm key={"update"} type={"update"} />
      </motion.div>
    </Container>
  );
}
